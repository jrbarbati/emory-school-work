...
*         ans_w = C[j + k];      
	...
	add.l   d1, d0
	muls    #4, d0
* Intermediate step, may be to move 0(a0,d0) into d0 first, then into ans_w (has implications in above answers as well if true).
	move.w  0(a0,d0), ans_w
...
*         ans_l = A[i] + B[k];  
	...
	ext.w   d0
	ext.l   d0
* Above 2 lines may not be needed, check with EGTAPI
	ext.l   d1
	add.l   d1, d0
	move.l  d0, ans_l
